...
const HOTEL_IMAGE = 'https://images.pexels.com/photos/30738386/pexels-photo-30738386.jpeg';
...
function StatCard({
  ...
  return (
    <View style={[styles.statCard, { backgroundColor: `${color}18`, borderColor: `${color}44` }]}>
      ...
      {sub ? <Text style={[styles.statSub, { color: theme.textSecondary }]}>{sub}</Text> : null}
    </View>
  );
}
...
export default function DashboardScreen() {
  ...
  const load = useCallback(async () => {
    if (isManager) {
      try {
        ...
        if (e?.message) { /* silently fall back */ }
        setSummary(null);
      }
    }
    setLoading(false);
  ...
  const myModules = ALL_MODULES.filter((m) => m.roles.includes(user?.role ?? ''));
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      <ScrollView
        ...
        {/* Hero Image */}
        <Image source={{ uri: HOTEL_IMAGE }} style={styles.heroImage} resizeMode="cover" />
        ...
        <View style={styles.content}>
          ...
          <View style={styles.userHeader}>
            ...
            <Pressable
              style={({ pressed }) => [styles.logoutBtn, { borderColor: theme.border, backgroundColor: `${theme.danger}18` }, pressed && { opacity: 0.7 }]}
          ...
          {loading ? (
            <ActivityIndicator size="large" color={theme.text} style={{ marginTop: Spacing.four }} />
          ) : (
            <>
              ...
              {isManager && summary !== null && (
                <View style={styles.section}>
                  ...
                  <View style={styles.statsGrid}>
                    <StatCard label="Total Staff" value={summary.totalStaff} color="#8b5cf6" theme={theme} />
                    <StatCard label="Total Rooms" value={summary.totalRooms} color="#10b981" theme={theme} />
                    <StatCard label="Room Bookings" value={summary.roomBookings} color="#3b82f6" theme={theme} />
                    <StatCard
                      label="Salary Paid"
                  ...
                  {summary.mostBookedRooms.length > 0 && (
                    <View style={[styles.roomsCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                      <Text style={[styles.roomsCardTitle, { color: theme.text }]}>Most Booked Rooms</Text>
                      {summary.mostBookedRooms.map((r) => (
                        <View key={r.roomNumber} style={styles.roomRow}>
                          <Text style={[styles.roomNumber, { color: theme.text }]}>Room {r.roomNumber}</Text>
                          <Text style={[styles.roomBookings, { color: theme.primary }]}>{r.bookings} bookings</Text>
                        </View>
                      ))}
                    </View>
                  )}
                ...
                <View style={styles.section}>
                  ...
                  {myModules.map((mod) => (
                    <Pressable
                      ...
                      style={({ pressed }) => [
                        styles.navCard,
                        { backgroundColor: theme.card, borderLeftColor: mod.color },
                        pressed && { opacity: 0.7 },
                      ]}
              ...
              {myModules.length === 0 && (
                <View style={styles.emptyState}>
                  <Text style={[styles.emptyTitle, { color: theme.text }]}>No modules available</Text>
                  <Text style={[styles.emptyText, { color: theme.textSecondary }]}>
                    Your role ({roleInfo.label}) does not have access to any management modules.
                  </Text>
                </View>
              )}
            </>
...
const styles = StyleSheet.create({
  ...
  logoutBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1, marginTop: 4 },
  ...
  statCard: { width: '47%', padding: Spacing.three, borderRadius: 12, borderWidth: 1, gap: 2 },
  statValue: { fontSize: 20, fontWeight: '800' },
  statLabel: { fontSize: 12, fontWeight: '500' },
  statSub: { fontSize: 11 },
  roomsCard: { borderRadius: 12, borderWidth: 1, padding: Spacing.three, gap: 8 },
  ...
  navCard: { padding: Spacing.three, borderRadius: 14, borderLeftWidth: 4, gap: 4, elevation: 1, shadowColor: '#0f1f2e', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 6 },
  navCardTitle: { fontSize: 15, fontWeight: '700' },
  navCardSub: { fontSize: 13 },
  navCardArrow: { fontSize: 18, fontWeight: '700', alignSelf: 'flex-end', marginTop: 2 },
  emptyState: { paddingTop: Spacing.six, alignItems: 'center', gap: Spacing.two },
  emptyTitle: { fontSize: 18, fontWeight: '600' },
  emptyText: { fontSize: 14, textAlign: 'center', paddingHorizontal: Spacing.four },
});