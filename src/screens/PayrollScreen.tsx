  ...
  return (
    ...
    >
          ...
          <View style={{ flexDirection: "row", gap: 8, marginTop: 2 }}>
            ...
            {item.staff?.position ? (
              <Text style={[styles.cardMeta, { color: theme.primary }]}>
                {item.staff.position.replaceAll("_", " ")}
              </Text>
            ) : null}
          </View>
        </View>
        <Text style={[styles.cardPeriod, { color: theme.textSecondary }]}>
      ...
      <View style={styles.cardGrid}>
        ...
        <GridItem
          label="Overtime"
          value={fmt(item.overtimePay)}
          theme={theme}
        />
        <GridItem
          label="Deductions"
          value={fmt(item.deductions)}
          theme={theme}
        />
        <View
          style={[
    ...
    >
      <View style={styles.overlay}>
        <View style={[styles.sheet, { backgroundColor: theme.card }]}>
          ...
          <ScrollView
            style={[styles.staffPicker, { borderColor: "#e5e7eb" }]}
            nestedScrollEnabled
          >
            {staff.map((s) => (
              <Pressable
                ...
                style={[
                  styles.staffOption,
                  staffId === s._id && { backgroundColor: "#3b82f622" },
                ]}
          ...
          <View style={styles.row}>
            <Pressable
              style={[styles.btn, styles.ghost, { borderColor: "#e5e7eb" }]}
              onPress={onClose}
            >
...
export default function PayrollScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (isManager) {
        const [payroll, staffList] = await Promise.all([
          getAllPayroll(),
          getStaff({ page: 0, size: 200 }),
        ]);
        setRows(payroll);
      ...
      Alert.alert(
        "Error",
        e?.response?.data?.message ?? e?.message ?? "Failed to load payroll.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <View style={[styles.header, { borderBottomColor: "#e5e7eb" }]}>
        <View>
          ...
          <Text style={[styles.title, { color: theme.text }]}>
            {isManager ? "Payroll Management" : "My Salary"}
          </Text>
        </View>
        {isManager && (
          <Pressable
