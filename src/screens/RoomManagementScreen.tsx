...
import { ActivityIndicator, Alert, FlatList, KeyboardAvoidingView, Modal, Platform, Image, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
            ...
            <SafeAreaView style={[styles.modalSafe, { backgroundColor: theme.background }]}>
                <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
                    ...
                    <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled">
                        ...
                        <Text style={[styles.fieldLabel, { color: theme.textSecondary }]}>Room Image</Text>
                        <Pressable
                        ...
                        >
                            ...
                            {!uploading && form.photoUrl ? <Image source={{ uri: form.photoUrl }} style={styles.imagePreview} resizeMode="cover" /> : null}
                            {!uploading && !form.photoUrl ? <Text style={[styles.imagePickerText, { color: theme.textSecondary }]}>📷 Tap to select image</Text> : null}
                        </Pressable>
                        {form.photoUrl ? (
                ...
                <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled">
                    ...
                    <View style={styles.rowFields}>
                        <View style={{ flex: 1 }}>
                            ...
                            <DateTimePickerField value={form.checkInDate} onChange={set("checkInDate")} mode="date" />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={[styles.fieldLabel, { color: theme.textSecondary }]}>Check-Out</Text>
                            <DateTimePickerField value={form.checkOutDate} onChange={set("checkOutDate")} mode="date" />
                        </View>
                    </View>
                    {error ? <Text style={styles.errorText}>{error}</Text> : null}
            ...
            <View style={[styles.tabBar, { borderBottomColor: "#e5e7eb" }]}>
                {(["rooms", "bookings"] as const).map((t) => (
                    <Pressable key={t} onPress={() => setTab(t)} style={[styles.tabBtn, tab === t && styles.tabActive]}>
                        <Text style={[styles.tabBtnText, { color: tab === t ? theme.text : theme.textSecondary }]}>{t === "rooms" ? "Rooms" : "Bookings"}</Text>
                    </Pressable>
                ))}
            </View>

            {loading ? <ActivityIndicator size="large" color={theme.text} style={{ marginTop: 40 }} /> : null}
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={[styles.card, { backgroundColor: "#f9fafb", borderColor: "#e5e7eb" }]}>
                            {item.photoUrl ? <Image source={{ uri: item.photoUrl }} style={styles.cardImage} resizeMode="cover" /> : null}
                            <View style={styles.cardRow}>
                                ...
                                <View
                                    style={[
                                        styles.badge,
                                        {
                                            backgroundColor: roomStatusColor(item.roomStatus) + "22",
                                        },
                                    ]}
                                >
                            ...
                            <Text style={[styles.cardMeta, { color: theme.textSecondary }]}>
                                Normal: Rs. {item.normalPrice?.toLocaleString()} · Weekend: Rs. {item.weekendPrice?.toLocaleString()}
                            </Text>
