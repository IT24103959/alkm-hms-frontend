  ...
  const loadData = useCallback(async () => {
    ...
    if (ticketsRes.status === "fulfilled") {
      ...
      setTickets(
        Array.isArray(raw)
          ? raw
          : ((raw as { content?: MaintenanceTicket[] }).content ?? []),
      );
    }
                ...
                >
                  ...
                  {stats.recurringIssues.map((issue) => (
                    <View key={issue._id} style={styles.recurringRow}>
                      <Text
                        style={[styles.recurringLabel, { color: theme.text }]}
                      >
                        {issue._id.replaceAll("_", " ")}
                      </Text>
                      <Text
                        style={[styles.recurringCount, { color: "#f97316" }]}
                      >
                        {issue.count}
                      </Text>
                    </View>
...
function MaintenanceFormModal({
  ...
  const setField = useCallback(
    (key: keyof typeof initialForm) => (v: string) =>
      setForm((f) => ({ ...f, [key]: v })),
    [],
  );
          ...
          >
            ...
            <FormField label="Issue Description" theme={theme}>
              <TextInput
                ...
                onChangeText={setField("issueDescription")}
              />
            </FormField>
            ...
            <FormField label="Resolution Notes (optional)" theme={theme}>
              <TextInput
                ...
                onChangeText={setField("resolutionNotes")}
              />
            </FormField>
            ...
            <FormField label="Parts Used (optional)" theme={theme}>
              <TextInput
                style={inputStyle}
                placeholder="e.g. Filter x2, Pipe joint"
                placeholderTextColor={theme.textSecondary}
                value={form.partsUsed}
                onChangeText={setField("partsUsed")}
              />
            </FormField>
